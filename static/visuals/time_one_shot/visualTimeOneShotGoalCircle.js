var visualTimeOneShot = function(divID, goal, wWidth, wHeight){
	...
	if (isComplete) {
		...
		var mArc = d3.svg.arc().innerRadius(mRadius).outerRadius(radius).startAngle(0);
	...
	$(window).resize(function() {
		...
		mRadius = .85*radius; //meter inner radius
		svgWidth = wWidth;
		...
		arc = d3.svg.arc().innerRadius(0).outerRadius(radius*.8).startAngle(0);
		mainArc.attr("d", arc).attr("transform", "translate(" + width/2 + "," + height/2+")");
		mArc = d3.svg.arc().innerRadius(mRadius).outerRadius(radius).startAngle(0);
		meterArc.attr("d", mArc).attr("transform", "translate(" + width/2 + "," + height/2+")");
