...
from forms import (CollectMilestoneIDForm, CompletedButtonForm, CollectNoteJournalID, 
    MilestoneCollectNoteJournalID)
#one shot goal imports
from forms import (OneShotGoalForm, DeleteOneShotForm, OneShotNoteForm, DeleteOneShotNoteForm, 
    OneShotJournalForm, DeleteOneShotJournalForm)
#milestone goal imports
from models import (MilestoneGoal, Milestone)
from forms import (MilestoneGoalForm, DeleteMilestoneGoalForm, MilestoneForm, MilestoneFormSet, 
    SubMilestoneFormSet, DeleteMilestoneForm, MilestoneGoalNoteForm, DeleteMilestoneGoalNoteForm, 
    MilestoneGoalJournalForm, DeleteMilestoneGoalJournalForm, MilestoneNoteForm, 
    DeleteMilestoneNoteForm, MilestoneJournalForm, DeleteMilestoneJournalForm)
#time one shot goal imports
from forms import (TimeOneShotGoalForm, DeleteTimeOneShotForm, TimeOneShotNoteForm, 
    DeleteTimeOneShotNoteForm, TimeOneShotJournalForm, DeleteTimeOneShotJournalForm)
#time milestone goal imports
from models import (TimeMilestoneGoal, TimeMilestone)
from forms import (TimeMilestoneGoalForm, TimeMilestoneForm, TimeMilestoneFormSet, 
    TimeSubMilestoneFormSet, DeleteTimeMilestoneGoalForm, DeleteTimeMilestoneForm, 
    TimeMilestoneGoalNoteForm, DeleteTimeMilestoneGoalNoteForm, TimeMilestoneGoalJournalForm, 
    DeleteTimeMilestoneGoalJournalForm, TimeMilestoneNoteForm, DeleteTimeMilestoneNoteForm, 
    TimeMilestoneJournalForm, DeleteTimeMilestoneJournalForm)
#value goal imports
from forms import (ValueGoalForm, ValueUpdateForm, DeleteValueGoalForm, CollectUpdateIDForm, 
    ValueJournalForm, DeleteValueJournalForm, ValueNoteForm, DeleteValueNoteForm)
#progress goal imports
from forms import (ProgressGoalForm, ProgressUpdateForm, ProgressGoalForm, DeleteProgressGoalForm, 
    ProgressJournalForm, DeleteProgressJournalForm, ProgressNoteForm, DeleteProgressNoteForm)
#fitness goals imports
from models import (Fitness, FitnessJournal, FitnessNote, CardioEndurance, CardioEnduranceNote, 
    CardioEnduranceUpdate, StrengthEndurance, StrengthEnduranceNote, StrengthEnduranceUpdate,
    Strength, StrengthNote, StrengthUpdate, Speed, SpeedNote, SpeedUpdate)
from forms import (FitnessForm, FitnessJournalForm, FitnessNoteForm, DeleteFitnessForm, 
    DeleteFitnessJournalForm, DeleteFitnessNoteForm, CardioEnduranceForm, CardioEnduranceUpdateForm,
    CardioEnduranceNoteForm, DeleteCardioEnduranceForm, DeleteCardioEnduranceUpdateForm,
    DeleteCardioEnduranceNoteForm, StrengthEnduranceForm, StrengthEnduranceUpdateForm, 
    StrengthEnduranceNoteForm, DeleteStrengthEnduranceForm, DeleteStrengthEnduranceUpdateForm, 
    DeleteStrengthEnduranceNoteForm, StrengthForm, StrengthUpdateForm, StrengthNoteForm,
    DeleteStrengthForm, DeleteStrengthUpdateForm, DeleteStrengthNoteForm, SpeedForm, 
    SpeedUpdateForm, SpeedNoteForm, DeleteSpeedForm, DeleteSpeedUpdateForm, DeleteSpeedNoteForm)
...
def goals(request, username):
    ...
    fitnessgoals       = request.user.fitness.all()
    ...
    timemilestoneformset  = TimeMilestoneFormSet(prefix='timemilestone', 
                                                 queryset=TimeMilestone.objects.none())
    ...
    if request.method == 'POST':
        if 'osgoalSub' in request.POST:
            ...
            if milestonegoalform.is_valid():
                ...
                milestoneformset = MilestoneFormSet(request.POST, request.FILES, prefix='milestone',
                    queryset=Milestone.objects.none(), instance=msGoalObject)
            ...
            if timemilestonegoalform.is_valid():
                ...
                timemilestoneformset = TimeMilestoneFormSet(request.POST, request.FILES, 
                    prefix='timemilestone', queryset=TimeMilestone.objects.none(), 
                    instance=tmsGoalObject)
            ...
            if progressgoalform.is_valid():
                ...
                pGoal.save()
                return HttpResponseRedirect('/user/%s/'%request.user.username)
        elif 'fitnessSub' in request.POST:
            fitnessform = FitnessForm(request.POST)
            if fitnessform.is_valid():
                fGoal = fitnessform.save(commit=False)
                fGoal.owner = user
                fGoal.visual = 'Fitness'
                fGoal.save()
                return HttpResponseRedirect('/user/%s/'%request.user.username)
    return render(request, 'goals/goals.html', 
        {
            'user' : user, 
            'oneshotgoalcount'     : oneshotgoalcount,
            'oneshotgoals'         : oneshotgoals, 
            'oneshotgoalform'      : oneshotgoalform, 
            ...
            'timemilestonegoalform': timemilestonegoalform, 
            'timemilestoneformset' : timemilestoneformset, 
            'timemilestonegoals'   : timemilestonegoals,
...
def osgoals(request, username, title, id):
    ...
    gl = {
        ...
        "goal_date_created"  : getDateTime(goal.date_created), 
        "goal_date_completed": getDateTime(goal.date_completed), 
        "goal_last_updated"  : getDateTime(goal.last_updated), 
        "goal_id"            : goal.id
    }
    ...
    def journalProcess(jnl):
        jnl = {
            'journal_entry': jnl.entry, 
            'journal_title': jnl.title, 
            'journal_date': getDateTime(jnl.date), 
            'journal_id': jnl.id
        }
        return jnl
    journals = []
    ...
    if request.method == 'POST':
        ...
        #Display subs
            ...
            if collectnotejournalid.is_valid():
                ...
                deleteoneshotjournalform = DeleteOneShotJournalForm(request.POST, 
                    instance=journalToDelete)
                if deleteoneshotjournalform.is_valid():
                    journalToDelete.delete()
                    return HttpResponseRedirect(link)
    return render(request, 'goals/oneshotgoals.html', 
        {
            'user'                    : user, 
            'title'                   : title, 
            'goalJSON'                : mark_safe(goalJSON), 
            'goal'                    : goal, 
            'oneshotjournalform'      : oneshotjournalform, 
            'oneshotnoteform'         : oneshotnoteform, 
            'editoneshotform'         : editoneshotform, 
            'oneshotjournal'          : oneshotjournal, 
            'noteJSON'                : mark_safe(noteJSON), 
            'deleteoneshotform'       : deleteoneshotform, 
            'completedbuttonform'     : completedbuttonform,
            'visual'                  : visual, 
            'uncompleteoneshotform'   : uncompleteoneshotform, 
            'link'                    : str(link), 
            'editoneshotnoteform'     : editoneshotnoteform,
            'collectnotejournalid'    : collectnotejournalid, 
            'deleteoneshotnoteform'   : deleteoneshotnoteform, 
            'deleteoneshotjournalform': deleteoneshotjournalform,
            'editoneshotjournalform'  : editoneshotjournalform, 
            'journalJSON'             : mark_safe(journalJSON)
        }
    )
...
def msgoals(request, username, title, id):
    ...
    milestoneformset        = MilestoneFormSet(prefix='milestone', 
                                               queryset=Milestone.objects.none())
    submilestoneform        = MilestoneForm()
    submilestoneformset     = SubMilestoneFormSet(prefix='submilestone', 
                                                  queryset=Milestone.objects.none())
    ...
    def find_SubMilestones(ms):
        ...
        obj = {
            ...
            "milestone_private"       : ms.private, 
            "milestone_completed"     : ms.completed, 
            "milestone_date_completed": getDateTime(ms.date_completed), 
            "milestone_id"            : ms.id, 
            "milestone_parent_id"     : id, 
            "milestone_is_sub"        : isSub, 
            "milestone_last_updated"  : getDateTime(ms.last_updated), 
            "milestone_date_created"  : getDateTime(ms.date_created), 
            "submilestones"           : [], 
            'milestone_journal'       : [], 
            'milestone_notes'         : []
        }
    ...
    if request.method == 'POST':
        if 'editMSGSub' in request.POST:
            ...
            milestoneformset = MilestoneFormSet(request.POST, request.FILES, prefix='milestone', 
                queryset=Milestone.objects.none(), instance=goal)
            if milestoneformset.is_valid():
            ...
            if collectmilestoneidform.is_valid():
                ...
                submilestoneformset = SubMilestoneFormSet(request.POST, request.FILES, 
                    prefix='submilestone', queryset=Milestone.objects.none(), instance=clean_ms)
        ...
        #goal notes and journal
            ...
            if milestonecollectnotejournalid.is_valid():
                ...
                deletemilestonegoalnoteform = DeleteMilestoneGoalNoteForm(request.POST, 
                    instance=noteToDelete)
            ...
            if milestonecollectnotejournalid.is_valid():
                ...
                editmilestonegoaljournalform = MilestoneGoalJournalForm(request.POST, 
                    instance=journalObj)
            ...
            if milestonecollectnotejournalid.is_valid():
                ...
                deletemilestonegoaljournalform = DeleteMilestoneJournalForm(request.POST, 
                    instance=journalToDelete)
        ...
        #milestone journal and notes
            ...
            if milestonecollectnotejournalid.is_valid():
                ...
                deletemilestonenoteform = DeleteMilestoneNoteForm(request.POST, 
                    instance=noteToDelete)
            ...
            if milestonecollectnotejournalid.is_valid():
                ...
                deletemilestonejournalform = DeleteMilestoneJournalForm(request.POST, 
                    instance=journalToDelete)
                if deletemilestonejournalform.is_valid():
                    journalToDelete.delete()
                    return HttpResponseRedirect(link)
    return render(request, 'goals/milestonegoals.html', 
        {
            'goalJSON'                      : mark_safe(goalJSON), 
            'test_milestones1'              : mark_safe(test_milestones1), 
            'test_milestones'               : test_milestones, 
            'user'                          : user, 
            'title'                         : title, 
            'goal'                          : goal,
            'milestones'                    : milestones, 
            'deletemilestonegoalform'       : deletemilestonegoalform, 
            'editmilestonegoalform'         : editmilestonegoalform, 
            'milestoneformset'              : milestoneformset, 
            'submilestoneformset'           : submilestoneformset, 
            'collectmilestoneidform'        : collectmilestoneidform,
            'deletemilestoneform'           : deletemilestoneform, 
            'editmilestoneform'             : editmilestoneform,
            'completedbuttonform'           : completedbuttonform, 
            'visual'                        : visual, 
            'alljournalsJSON'               : mark_safe(alljournalsJSON),
            'milestonegoalnoteform'         : milestonegoalnoteform, 
            'editmilestonegoalnoteform'     : editmilestonegoalnoteform,
            'deletemilestonegoalnoteform'   : deletemilestonegoalnoteform, 
            'milestonegoaljournalform'      : milestonegoaljournalform,
            'editmilestonegoaljournalform'  : editmilestonegoaljournalform, 
            'deletemilestonegoaljournalform': deletemilestonegoaljournalform,
            'milestonenoteform'             : milestonenoteform, 
            'editmilestonenoteform'         : editmilestonenoteform, 
            'deletemilestonenoteform'       : deletemilestonenoteform,
            'milestonejournalform'          : milestonejournalform, 
            'editmilestonejournalform'      : editmilestonejournalform,
            'deletemilestonejournalform'    : deletemilestonejournalform, 
            'milestonecollectnotejournalid' : milestonecollectnotejournalid
        }
    )
...
def tosgoals(request, username, title, id):
    ...
    gl = {
        ...
        "goal_private"       : goal.private, 
        "goal_completed"     : goal.completed, 
        "goal_complete_by"   : getDateTime(goal.complete_by), 
        "goal_date_created"  : getDateTime(goal.date_created), 
        "goal_date_completed": getDateTime(goal.date_completed), 
        "goal_last_updated"  : getDateTime(goal.last_updated), 
        "goal_id"            : goal.id
    ...
    if request.method == 'POST':
        ...
        if 'editSub' in request.POST:
            ...
            if completedbuttonform.is_valid():
                date = completedbuttonform.cleaned_data['date_completed']
                timeoneshotgoalform = TimeOneShotGoalForm({'title':goal.title, 
                    'complete_by': goal.complete_by}, instance=goal)
                if timeoneshotgoalform.is_valid():
        ...
        #Display subs
            ...
            if collectnotejournalid.is_valid():
                ...
                deletetimeoneshotnoteform = DeleteTimeOneShotNoteForm(request.POST, 
                    instance=noteToDelete)
            ...
            if collectnotejournalid.is_valid():
                ...
                edittimeoneshotjournalform = TimeOneShotJournalForm(request.POST, 
                    instance=journalObj)
            ...
            if collectnotejournalid.is_valid():
                ...
                deletetimeoneshotjournalform = DeleteTimeOneShotJournalForm(request.POST, 
                    instance=journalToDelete)
                if deletetimeoneshotjournalform.is_valid():
                    journalToDelete.delete()
                    return HttpResponseRedirect(link)
    return render(request, 'goals/timeoneshotgoals.html', 
        {
            'user'                        : user, 
            'title'                       : title, 
            'goalJSON'                    : mark_safe(goalJSON), 
            'goal'                        : goal, 
            'edittimeoneshotform'         : edittimeoneshotform, 
            'deletetimeoneshotform'       : deletetimeoneshotform, 
            'completedbuttonform'         :completedbuttonform, 
            'visual'                      : visual, 
            'noteJSON'                    : mark_safe(noteJSON), 
            'journalJSON'                 : mark_safe(journalJSON), 
            'timeoneshotnoteform'         : timeoneshotnoteform, 
            'edittimeoneshotnoteform'     : edittimeoneshotnoteform, 
            'deletetimeoneshotnoteform'   : deletetimeoneshotnoteform, 
            'timeoneshotjournalform'      : timeoneshotjournalform, 
            'edittimeoneshotjournalform'  : edittimeoneshotjournalform,
            'deletetimeoneshotjournalform': deletetimeoneshotjournalform, 
            'collectnotejournalid'        : collectnotejournalid
        }
    )                
...
def tmsgoals(request, username, title, id):
    ...
    milestoneformset        = TimeMilestoneFormSet(prefix='timemilestone', 
                                                   queryset=TimeMilestone.objects.none())
    submilestoneform        = TimeMilestoneForm()
    submilestoneformset     = TimeSubMilestoneFormSet(prefix='timesubmilestone', 
                                                      queryset=TimeMilestone.objects.none())
    collectmilestoneidform  = CollectMilestoneIDForm()
    ...
    def find_SubMilestones(ms):
        ...
        obj = {
            ...
            "milestone_completed"     : ms.completed, 
            "milestone_date_completed": getDateTime(ms.date_completed), 
            "milestone_complete_by"   : getDateTime(ms.complete_by), 
            "milestone_id"            : ms.id, 
            "milestone_parent_id"     : id, 
            "milestone_is_sub"        : isSub, 
            "milestone_last_updated"  : getDateTime(ms.last_updated), 
    ...
    if request.method == 'POST':
        if 'editMSGSub' in request.POST:
            ...
            deletemilestonegoalform = DeleteTimeMilestoneGoalForm(request.POST, 
                instance=goalToDelete)
            if deletemilestonegoalform.is_valid():
                goalToDelete.delete()
                return HttpResponseRedirect('/user/%s/'%request.user.username)
        elif 'deleteMSSub' in request.POST:
            ...
            milestoneformset = TimeMilestoneFormSet(
                request.POST, request.FILES, prefix='timemilestone', 
                queryset=TimeMilestone.objects.none(), instance=goal
            )
            if milestoneformset.is_valid():
                newobj = milestoneformset.save()
                incomplete = True
            ...
            if collectmilestoneidform.is_valid():
                ...
                submilestoneformset = TimeSubMilestoneFormSet(request.POST, request.FILES, 
                    prefix='timesubmilestone', queryset=TimeMilestone.objects.none(), 
                    instance=clean_ms)
                if submilestoneformset.is_valid():
                    incomplete = True
                    subfm = submilestoneformset.save()
            ...
            if collectmilestoneidform.is_valid():
                ...
                if completedbuttonform.is_valid():
                    ...
                    if milestone_isGoal:
                        for ms in allMS:
                            completeMS(ms, date)
                        milestonegoalform = TimeMilestoneGoalForm({'title':goal.title, 
                            'complete_by':goal.complete_by}, instance=goal)
                        if milestonegoalform.is_valid():
                            msgform = milestonegoalform.save(commit=False)
                            msgform.completed = True
        ...
        #goal notes and journal
            ...
            if milestonecollectnotejournalid.is_valid():
                ...
                edittimemilestonegoalnoteform = TimeMilestoneGoalNoteForm(request.POST, 
                    instance=noteObj)
            ...
            if milestonecollectnotejournalid.is_valid():
                ...
                deletetimemilestonegoalnoteform = DeleteTimeMilestoneGoalNoteForm(request.POST, 
                    instance=noteToDelete)
            ...
            if milestonecollectnotejournalid.is_valid():
                ...
                edittimemilestonegoaljournalform = TimeMilestoneGoalJournalForm(request.POST, 
                    instance=journalObj)
            ...
            if milestonecollectnotejournalid.is_valid():
                ...
                deletetimemilestonegoaljournalform = DeleteTimeMilestoneJournalForm(request.POST, 
                    instance=journalToDelete)
        ...
        #milestone journal and notes
            ...
            if milestonecollectnotejournalid.is_valid():
                ...
                deletetimemilestonenoteform = DeleteTimeMilestoneNoteForm(request.POST, 
                    instance=noteToDelete)
            ...
            if milestonecollectnotejournalid.is_valid():
                ...
                edittimemilestonejournalform = TimeMilestoneJournalForm(request.POST, 
                    instance=journalObj)
            ...
            if milestonecollectnotejournalid.is_valid():
                ...
                deletetimemilestonejournalform = DeleteTimeMilestoneJournalForm(request.POST, 
                    instance=journalToDelete)
                if deletetimemilestonejournalform.is_valid():
                    journalToDelete.delete()
                    return HttpResponseRedirect(link)
    return render(request, 'goals/timemilestonegoals.html', 
        {
            'goalJSON': mark_safe(goalJSON), 
            'test_milestones1'                  : mark_safe(test_milestones1), 
            'test_milestones'                   : test_milestones, 
            ...
            'alljournalsJSON'                   : mark_safe(alljournalsJSON),
            'timemilestonegoalnoteform'         : timemilestonegoalnoteform, 
            'edittimemilestonegoalnoteform'     : edittimemilestonegoalnoteform,
            'deletetimemilestonegoalnoteform'   : deletetimemilestonegoalnoteform, 
            'timemilestonegoaljournalform'      : timemilestonegoaljournalform,
            'edittimemilestonegoaljournalform'  : edittimemilestonegoaljournalform, 
            'deletetimemilestonegoaljournalform': deletetimemilestonegoaljournalform,
            'timemilestonenoteform'             : timemilestonenoteform, 
            'edittimemilestonenoteform'         : edittimemilestonenoteform, 
            'deletetimemilestonenoteform'       : deletetimemilestonenoteform, 
            'timemilestonejournalform'          : timemilestonejournalform, 
            'edittimemilestonejournalform'      : edittimemilestonejournalform, 
            'deletetimemilestonejournalform'    : deletetimemilestonejournalform, 
            'milestonecollectnotejournalid'     : milestonecollectnotejournalid
        }
    )
...
def vgoals(request, username, title, id):
    ...
    def processUpdate(up, total):
        ...
        temp = total+up.value
        upda = {
            "update_value"       : str(up.value), 
            "update_description" : up.description, 
            "update_date_created": getDateTime(up.date_created), 
            "update_id"          : up.id,
            "update_last_updated": getDateTime(up.last_updated), 
            "update_total"       : str(temp),
            "update_nozero"      : (zeroOut(str(temp))), 
            "update_value_nozero": (zeroOut(str(up.value)))
        }
    ...
    gl = {
        "goal_title"         : goal.title, 
        "goal_description"   : goal.description, 
        "goal_determinate"   : goal.determinate, 
        "goal_valueType"     : goal.valueType, 
        "goal_startValue"    : str(goal.startValue), 
        "goal_endValue"      : str(goal.endValue), 
        "goal_private"       : goal.private, 
        "goal_completed"     : goal.completed, 
        "goal_complete_by"   : getDateTime(goal.complete_by), 
        "goal_date_completed": getDateTime(goal.date_completed), 
        ...
        "goal_date_created"  : getDateTime(goal.date_created), 
        "goal_id"            : goal.id, 
        "goal_nozero"        : (zeroOut(str(goal.startValue))), 
    ...
    if request.method == 'POST':
        ...
        #Display subs
            ...
            if collectnotejournalid.is_valid():
                ...
                deletevaluejournalform = DeleteValueJournalForm(request.POST, 
                    instance=journalToDelete)
                if deletevaluejournalform.is_valid():
                    journalToDelete.delete()
                    return HttpResponseRedirect(link)
    return render(request, 'goals/valuegoals.html', 
        {
            'user'                  : user, 
            'title'                 : title, 
            'goalJSON'              : mark_safe(goalJSON), 
            'updates'               : mark_safe(updates), 
            'goal'                  : goal, 
            'editvaluegoalform'     : editvaluegoalform, 
            'deletevaluegoalform'   : deletevaluegoalform, 
            'visual'                : visual, 
            'valueupdateform'       : valueupdateform,
            ...
            'link'                  : str(link), 
            'valuenoteform'         : valuenoteform, 
            'editvaluenoteform'     : editvaluenoteform, 
            'deletevaluenoteform'   : deletevaluenoteform, 
            'valuejournalform'      : valuejournalform, 
            'deletevaluejournalform': deletevaluejournalform, 
            'editvaluejournalform'  : editvaluejournalform, 
            'collectnotejournalid'  : collectnotejournalid, 
            'journalJSON'           : mark_safe(journalJSON), 
            'noteJSON'              : mark_safe(noteJSON)
        }
    ...
    if request.method == 'POST':
        ...
        #Display subs
            ...
            if collectnotejournalid.is_valid():
                ...
                deleteprogressnoteform = DeleteProgressNoteForm(request.POST, 
                    instance=noteToDelete)
            ...
            if collectnotejournalid.is_valid():
                ...
                deleteprogressjournalform = DeleteProgressJournalForm(request.POST, 
                    instance=journalToDelete)
                if deleteprogressjournalform.is_valid():
                    journalToDelete.delete()
                    return HttpResponseRedirect(link)
    return render(request, 'goals/progressgoals.html', 
        {
            'user'                     : user, 
            'title'                    : title, 
            'goalJSON'                 : mark_safe(goalJSON), 
            'updates'                  : mark_safe(updates), 
            'goal'                     : goal, 
            'editprogressgoalform'     : editprogressgoalform, 
            'deleteprogressgoalform'   : deleteprogressgoalform, 
            'visual'                   : visual,
            'progressupdateform'       : progressupdateform, 
            'editupdateform'           : editupdateform, 
            ...
            'presentValue'             : presentValue,
            'progressnoteform'         : progressnoteform, 
            'editprogressnoteform'     : editprogressnoteform, 
            'deleteprogressnoteform'   : deleteprogressnoteform, 
            'progressjournalform'      : progressjournalform, 
            'deleteprogressjournalform': deleteprogressjournalform, 
            'editprogressjournalform'  : editprogressjournalform, 
            'collectnotejournalid'     : collectnotejournalid, 
            'journalJSON'              : mark_safe(journalJSON), 
            'noteJSON'                 : mark_safe(noteJSON)
        }
...
@login_required
def fitnessgoals(request, username, title, id):
    """
    Displays a progress goal
    """
    user = request.user
    ...
    #goal variables
    goal           = request.user.fitness.get(id=id)
    fitnessnotes   = goal.fitnessnote.all()
    fitnessjournal = goal.fitnessjournal.all().order_by("date")
    
    #Sub Goal values
    cardioEnduranceGoals   = goal.cardioendurance.all()
    strengthEnduranceGoals = goal.strengthendurance.all()
    strengthGoals          = goal.strength.all()
    speedGoals             = goal.speed.all()
    
    #visual
    visual = goal.visual
    link   = ('/user/%s/fitness/%s%d/'%(request.user.username, title, goal.id))
    
    #Form variables
    ceform       = CardioEnduranceForm()
    ceupdateform = CardioEnduranceUpdateForm()
    seform       = StrengthEnduranceForm()
    seupdateform = StrengthEnduranceUpdateForm()
    stform       = StrengthForm()
    stupdateform = StrengthUpdateForm()
    spform       = SpeedForm()
    spupdateform = SpeedUpdateForm()
    
    #Note and Journal Forms
    fitnessnoteform    = FitnessNoteForm()
    fitnessjournalform = FitnessJournalForm()
    cenoteform         = CardioEnduranceNoteForm()
    senoteform         = StrengthEnduranceNoteForm()
    stnoteform         = StrengthNoteForm()
    speednoteform      = SpeedNoteForm()
    
    #Delete Forms
    deletespeedupdateform    = DeleteSpeedUpdateForm()
    deleteceform             = DeleteCardioEnduranceForm()
    deleteceupdateform       = DeleteCardioEnduranceUpdateForm()
    deleteseform             = DeleteStrengthEnduranceForm()
    deleteseupdateform       = DeleteStrengthUpdateForm()
    deletestrengthform       = DeleteStrengthForm()
    deletestrengthupdateform = DeleteStrengthUpdateForm()
    deletespeedform          = DeleteSpeedForm()
    deletespeedupdateform    = DeleteSpeedUpdateForm()
    deletefitnessnoteform    = DeleteFitnessNoteForm()
    deletefitnessjournalform = DeleteFitnessJournalForm()
    deletecenoteform         = DeleteCardioEnduranceNoteForm()
    deletesenoteform         = DeleteStrengthEnduranceNoteForm()
    deletestnoteform         = DeleteStrengthNoteForm()
    deletespeednoteform      = DeleteSpeedNoteForm()
    ...
    def notesProcess(nt):
        ...
        return nt
    notes = []
    for note in fitnessnotes:
        notes.append(notesProcess(note))
    noteJSON = json.dumps(notes)
    
    #Process CEGoals 
    #FIGURE OUT HOW TO CALCULATE PROGRESS
    def ceUpdateProcess(up):
        upda = {
            "duration"       : str(up.duration), 
            "distance"       : str(up.distance),
            "speed"          : str(up.speed), "description": up.description, 
            "date_created"   : getDateTime(up.date_created), "id": up.id, 
            "last_updated"   : getDateTime(up.last_updated), 
            "duration_nozero": (zeroOut(str(up.duration))),
            "distance_nozero": (zeroOut(str(up.distance))), 
            "speed_nozero"   : (zeroOut(str(up.speed)))}
        return upda
    
    def ceGoalProcess(ceg):
        ceupdates = ceg.cardioenduranceupdate.all()
        updates = []
        for upd in ceupdates:
            updates.append(ceUpdateProcess(upd))
        updatesJSON = json.dumps(updates)
        gl = {
            "exercise"    : ceg.exercise, 
            "description" : ceg.description,
            "durationType": ceg.durationType, 
            "distanceType": str(ceg.distanceType), 
            "speedType"   : str(ceg.speedType), 
            "private"     : ceg.private, 
            "last_updated": getDateTime(ceg.last_updated),
            "date_created": getDateTime(ceg.date_created), 
            "id"          : ceg.id, 
            "updates"     : updatesJSON
        }
        ceGoal = json.dumps(gl, cls=DecimalEncoder)
        return ceGoal
    
    ceGoals = []
    for ceg in cardioEnduranceGoals:
        ceGoals.append(ceGoalProcess(ceg))
    ceGoalsJSON = json.dumps(ceGoals)
    
    #Process SEGoals 
    #FIGURE OUT HOW TO CALCULATE PROGRESS
    def seUpdateProcess(up):
        upda = {
            "duration"         : str(up.duration), 
            "resistance"       : str(up.resistance),
            "description"      : up.description, 
            "date_created"     : getDateTime(up.date_created), 
            "id"               : up.id, 
            "last_updated"     : getDateTime(up.last_updated), 
            "duration_nozero"  : (zeroOut(str(up.duration))), 
            "resistance_nozero": (zeroOut(str(up.resistance)))
        }
        return upda
    
    def seGoalProcess(seg):
        seupdates = seg.strengthenduranceupdate.all()
        updates = []
        for upd in seupdates:
            updates.append(seUpdateProcess(upd))
        updatesJSON = json.dumps(updates)
        gl = {
            "exercise"         : seg.exercise, 
            "description"      : seg.description,
            "durationType"     : seg.durationType, 
            "distanceType"     : str(seg.distanceType), 
            "speedType"        : str(seg.speedType), 
            "private"          : seg.private, 
            "goal_last_updated": getDateTime(seg.last_updated),
            "date_created"     : getDateTime(seg.date_created), 
            "goal_id"          : seg.id, 
            "updates"          : updatesJSON
        }
        seGoal = json.dumps(gl, cls=DecimalEncoder)
        return seGoal
    
    seGoals = []
    for seg in strengthEnduranceGoals:
        seGoals.append(seGoalProcess(seg))
    seGoalsJSON = json.dumps(seGoals)
    
    #Process StGoals 
    #FIGURE OUT HOW TO CALCULATE PROGRESS
    def stUpdateProcess(up):
        upda = {
            "sets"        : str(up.sets), 
            "reps"        : str(up.reps), 
            "res"         : str(up.res),
            ...
            "id"          : up.id, 
            "last_updated": getDateTime(up.last_updated), 
            "sets_nozero" : (zeroOut(str(up.sets))), 
            "reps_nozero" : (zeroOut(str(up.reps))), 
            "res_nozero"  : (zeroOut(str(up.res)))
        }
        return upda
    
    def stGoalProcess(stg):
        stupdates = stg.strengthupdate.all()
        updates = []
        for upd in seupdates:
            updates.append(seUpdateProcess(upd))
        updatesJSON = json.dumps(updates)
        gl = {
            "exercise"         : stg.exercise, 
            "description"      : stg.description,
            "resType"          : stg.resType, 
            "private"          : stg.private, 
            "goal_last_updated": getDateTime(stg.last_updated),
            "date_created"     : getDateTime(stg.date_created), 
            "goal_id"          : stg.id, 
            "updates"          : updatesJSON
        }
        seGoal = json.dumps(gl, cls=DecimalEncoder)
    ...
    stGoals = []
    for stg in strengthGoals:
        stGoals.append(stGoalProcess(stg))
    stGoalsJSON = json.dumps(stGoals)
    
    #Process SpGoals 
    #FIGURE OUT HOW TO CALCULATE PROGRESS
    def spUpdateProcess(up):
        upda = {
            "dist"        : str(up.dist), 
            "time"        : str(up.time), 
            "description" : up.description, 
            ...
            "last_updated": getDateTime(up.last_updated), 
            "sets_nozero" : (zeroOut(str(up.sets))),
            "dist_nozero" : (zeroOut(str(up.dist))), 
            "time_nozero" : (zeroOut(str(up.time)))
        }
        return upda
    
    def spGoalProcess(spg):
        spupdates = spg.speedupdate.all()
        updates = []
        for upd in spupdates:
            #Add if else for constantDist attribute
            updates.append(spUpdateProcess(upd))
        updatesJSON = json.dumps(updates)
        gl = {
            "exercise"         : spg.exercise, 
            "description"      : spg.description, 
            "distanceType:"    : spg.distanceType, 
            "timeType"         : spg.timeType, 
            "private"          : spg.private, 
            "constantDist"     : spg.constantDist, 
            "baseDist"         : spg.baseDist, 
            "goal_last_updated": getDateTime(spg.last_updated), 
            "date_created"     : getDateTime(spg.date_created), 
            "id"               : spg.id, 
            "updates"          : updatesJSON
        }
        seGoal = json.dumps(gl, cls=DecimalEncoder)
    ...
    spGoals = []
    for spg in speedGoals:
        spGoals.append(spGoalProcess(spg))
    spGoalsJSON = json.dumps(spGoals)
        ...
        #Display subs
            ...
            if collectnotejournalid.is_valid():
                ...
                if deleteprogressjournalform.is_valid():
                    journalToDelete.delete()
                    return HttpResponseRedirect(link)
    return render(request, 'goals/fitness.html', 
        {
            'user' : user, 'title' : title, 'goalJSON': mark_safe(goalJSON)
        }
    )
